...
connected.
    ...
    curl http://localhost:35788/changed?files=style.css

    # notify using a longer path
    curl http://localhost:35788/changed?files=js/app.js

    # notify multiple changes, comma or space delimited
    curl http://localhost:35788/changed?files=index.html,style.css,docs/docco.css

Or you can bulk the information into a POST request, with body as a JSON array of files.

    curl -X POST http://localhost:35788/changed -d '{ "files": ["style.css", "app.js"] }'

    # from a JSON file
    node -pe 'JSON.stringify({ files: ["some.css", "files.css"] })' > files.json
    curl -X POST -d @files.json http://localhost:35788

As for the livereload client, you need to install the browser extension:
http://feedback.livereload.com/knowledgebase/articles/86242-how-do-i-install-and-use-the-browser-extensions-
(**note**: you need to listen on port 35788 to be able to use with your
brower extension)
...
```js
var port = process.env.LR_PORT || process.env.PORT || 35788;
...
the LiveReload standard one: `35788`. Otherwise, you won't be able to rely
...
- `port`          - Livereload port (defaults to `35788`)
