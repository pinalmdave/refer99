...
gulp.task('styles', function() {
  ...
  var cssStream = gulp.src('app/styles/custom.scss')
    .pipe(cache('styles'))
    .pipe(plugins.sass(options))
    .pipe(plugins.remember('styles'))
    .on('error', function(err) {
  ...
  return streamqueue({
      objectMode: true
    }, ionicStream, sassStream, cssStream)
...
gulp.task('test-e2e', ['default'], function() {
  ...
  return new Promise(function(resolve, reject) {
    ...
    var webdriverUpdate = spawn('node', [webdriverBinary, 'update'], {
        stdio: 'inherit'
      })
      .once('close', function() {
        var webdriverProcess = spawn('node', [webdriverBinary, 'start'], {
          stdio: 'inherit'
        });
